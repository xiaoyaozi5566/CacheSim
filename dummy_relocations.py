...
def relocate(tree, index):
    ...
    global relocation_counter
    relocation_counter = relocation_counter + 1
    if index <= phy_assoc:
...
for i in range(num_rounds):
    ...
    relocation_counter = 0
    while relocation_counter < num_relocations:
        victim_addr = random.randint(0, tag_range)
...
cax = ax.matshow(result, interpolation='nearest', aspect = ratio)
fig.colorbar(cax)
plt.savefig('hit_miss_'+ str(len(attacker_addr)) + '_reloc.pdf', bbox_inches='tight')
plt.close()
